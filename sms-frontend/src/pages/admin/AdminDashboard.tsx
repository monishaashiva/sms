...
export default function AdminDashboard() {
  ...
  //  NEW STATE (safe default)
  const [avgAttendance, setAvgAttendance] = useState<number>(
...
const [recentStudentsData, setRecentStudentsData] = useState<any[]>([]);
...
useEffect(() => {
  api.get("/dashboard/summary")
    .then(res => {
      ...
      setRecentStudentsData(res.data.recentStudents);
    })
    .catch(console.error);
}, []);
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="glass-card p-0 overflow-hidden">
            ...
            <div className="divide-y divide-border/50">
              {recentStudentsData.map((student, index) => (
                <motion.div 
                  key={student.id} 
