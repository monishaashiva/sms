...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      const [dashboardRes, studentsRes, notificationsRes] = await Promise.all([
      ...
      >
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard
            ...
            value={(stats?.overview?.totalStudents || 0).toLocaleString('en-IN')}
            icon={Users}
            variant="primary"
          ...
          <StatCard
            ...
            value={formatCurrencyShort(stats?.finance?.feesCollected || 0)}
            icon={IndianRupee}
            variant="warning"
            delay={3}
          />
          ...
          <div className="glass-card lg:col-span-2">
            ...
            <div className="flex items-end gap-8 flex-wrap">
              <div className="space-y-1">
                ...
                <p className="text-3xl font-bold text-success">{formatCurrencyShort(stats?.finance?.feesCollected || 0)}</p>
              </div>
              <div className="space-y-1">
                <p className="text-sm text-muted-foreground">Pending</p>
                <p className="text-3xl font-bold text-warning">{formatCurrencyShort(stats?.finance?.pendingFees || 0)}</p>
              </div>
              <div className="flex-1 min-w-[200px]">
                <div className="h-4 bg-muted/50 rounded-full overflow-hidden backdrop-blur-sm">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${((stats?.finance?.feesCollected || 0) / ((stats?.finance?.feesCollected || 0) + (stats?.finance?.pendingFees || 1)) * 100)}%` }}
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  <span className="font-semibold text-foreground">
                    {(((stats?.finance?.feesCollected || 0) / ((stats?.finance?.feesCollected || 0) + (stats?.finance?.pendingFees || 1))) * 100).toFixed(1)}%
              ...
              {recentStudents.map((student, index) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">Class {student.class?.name || student.class}</p>
                  </div>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground/70 mt-2">{new Date(notification.createdAt).toLocaleDateString()}</p>
                    </div>
                  </div>
                </motion.div>
              ))}
